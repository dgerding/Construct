...
namespace ConstructTcpServer
{
    ...
    public class SocketConnectionProcessor
    {
        ...
        readonly Encoding _textEncoding = Encoding.ASCII;
        readonly SortedList<string, GameContext> _socketContext;
        readonly int _iReceiveTimeout;
        readonly Action _abortTcpServer;
        readonly Action<SocketConnectionProcessor> _finishAction;

        Socket _clientSocket;
        ProducerConsumerQueue<string[]> _messageQueue;
        ConstructGameProcessor _gameProcessor;
        GameContext _context;
        List<TcpServerContext> _colAudioSockets;
        List<TcpServerContext> _colVideoSockets;
        ...
        public SocketConnectionProcessor(Socket clientSocket, SortedList<string, GameContext> socketContext, Action abortTcpServer, Action<SocketConnectionProcessor> finishAction)
        {
            ...
            _colAudioSockets = null;
            _colVideoSockets = null;
                ...
                try
                {
                    while (bContinue && _clientSocket.Connected)
                    {
                        ...
                        else
                        {
                            ...
                            colMessages = string.Format("{0}{1}", strMessage, strNewMessage).Split('\r');
            ...
            if (colMessageParts.Length > 0)
            {
                switch (colMessageParts[0])
                {
                    ...
                    case "CountdownStarted":
                        //External app startup like video and audio used to go here...
                        break;
                    case "StartPlay":
                        LogMsg.Debug(strMessage);
                        //Console.WriteLine(strMessage);
                    ...
                    case "StartApp":
                        ...
                        if (!string.IsNullOrEmpty(strAppFileAndPath))
                        {
                            ...
                            {
                                processInfo.Arguments += string.Format("{0} ", colMessageParts[iIndex]);
                            }
