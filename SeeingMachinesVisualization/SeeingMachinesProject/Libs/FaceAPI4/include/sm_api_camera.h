...
/*! @brief Register types of cameras for detection and usage. 

    For hardware cameras, this will scan the system and attempt to run to each 
    instance of the camera type to confirm if the camera is operational. 
    ...
    @param type Set this to one of the possible smCameraType values.
                - For the camera type @ref smCameraType "SM_API_CAMERA_TYPE_WDM"
                  the function uses the system device enumerator to obtain a list 
                  of all WDM compatible cameras, and then attempts to run to each 
                  camera to confirm if it is functional.
                - For the camera type @ref smCameraType "SM_API_CAMERA_TYPE_PTGREY"
                  the function queries the PointGrey driver for connected 
                  PointGrey cameras.
                  - For the camera type @ref smCameraType "SM_API_CAMERA_TYPE_FILE" 
                  the function will initialize code for reading from various movie 
                  formats. This camera type only needs manual registration using
                  this function if the camera will be created from a worker thread.
                - The type @ref smCameraType "SM_API_CAMERA_TYPE_IMAGE_PUSH" does 
    ...
    - For custom cameras, you will need to create an @ref smCameraType "SM_API_CAMERA_TYPE_IMAGE_PUSH" camera, 
      ...
      Refer to the ImagePushCamera class in the C++ wrapper library, and the 
      TestAppImagePush* sample projects.
...
/*! @brief Creates a camera that obtains its image data from a movie file.
    ...
    @param loop_playback Whether to rewind and continue playing when at EOF. If false, any engine that uses the camera will automatically stop tracking (enter the idle state) when the EOF condition is reached.
    @param settings Optional settings for the camera. If 0, default settings will be used. The format_index field is ignored.
    ...
    @note If calling this function from within a worker thread, be sure to call smCameraRegisterType(SM_API_CAMERA_TYPE_FILE) beforehand. */
SM_API(smReturnCode) smCameraCreateFile(smStringHandle filename, smBool loop_playback, const smCameraSettings *settings, smCameraHandle *file_camera_handle);
...
/*! @brief Push an smImage into an "Image Push" smCameraHandle
    ...
    @see
    - smCameraImagePushBlock()
    - smImageHandle
    - smImageInfo    
    - smRegisterImageReleaseCallback() 
    - smCameraCreateImagePush() */
SM_API(smReturnCode) smCameraImagePush(smCameraHandle push_camera_handle, const smCameraVideoFrame* video_frame, const smCameraLensParams* lens_params, const smImageRect *src_rect);
...
/*! @brief Push an smImage into an "Image Push" smCameraHandle, blocking if the fifo is full, until the fifo is
    ...
    @param lens_params Optional argument (can be 0) that can be used to override any existing lens-parameters the camera is using.
           This argument is only necessary if the lens is changing dynamically (e.g motorized zoom lens).
    @param src_rect Optional argument (can be 0) describing the position and size of the region inside the image dimensions of the 
           image used to calibrate the sensor, where the image being pushed is obtained from.\n
           This is typically used when a camera driver is able to provide a region-of-interest (ROI)
           sub-image. This argument then defines where that sub-image is located.\n
           It can also be used to specify a ROI image that has also been resized / decimated by the camera driver.
    ...
    - smRegisterImageReleaseCallback() 
    - smCameraCreateImagePush() */
SM_API(smReturnCode) smCameraImagePushBlock(smCameraHandle push_camera_handle, int timeout_ms, const smCameraVideoFrame* video_frame, const smCameraLensParams* lens_params, const smImageRect *src_rect);

/*! @brief Extension to smCameraImagePushBlock() with extra argument to add user data of any type. 

    The user data will appear in the output data structure: smEngineData.user_data.

    The engine may buffer several frames of video depending on CPU load conditions. Do not to assume that
    for every image pushed into the engine, output data will be immediately produced for that frame.

    This function is provided to allow any user data associated with the input image to be kept in sync with the input video. 

    In this way, the engine is able to be an element in any video-processing pipeline.

    @verbatim
    {Video + Data} => [Engine] => {Video + Data} + Tracking Results
    @endverbatim

    For example, a application wishes to use a stereo camera. Tracking can be performed on either left or right image streams,
    however the tracking introduces a variable latency of potentially several frames. In order to keep the left and right image
    streams synchronized, the second stream is passed through the engine using the smCameraImagePushBlockEx() function, specifically
    by casting the second image to a void * and passing it to the @a user_data argument. A separate thread then calls 
    smEngineDataWaitNext() to obtain the smEngineOutputData as it is produced, and casts the @a smEngineData.user_data field back
    to the original image type.
    ...
    @param src_rect Optional argument (can be 0) describing the position and size of the region inside the image dimensions of the 
           ...
           parameter is set to the offset in the (1280,1024) image.
           The following @ref sm_api_image_push_camera_roi "diagram" illustrates the @a src_rect concept.
    @param user_data A pointer to memory that the user is required to maintain. The pointer is provided in smEngineData.user_data
           when data is produced by the engine. This allows any user_data to be kept in sync with the input video.
    ...
    - smRegisterImageReleaseCallback() 
    - smCameraCreateImagePush() */
SM_API(smReturnCode) smCameraImagePushBlockEx(smCameraHandle push_camera_handle, int timeout_ms, const smCameraVideoFrame* video_frame, const smCameraLensParams* lens_params, const smImageRect *src_rect, void *user_data);

/*! @brief Loads the intrinsic lens parameters from filename.
    
    Lens parameter files can be generated by the API tool "camcal.exe", which is part of the API distribution under API/Tools/CamCal.
    The function loads the intrinsic parameters which are usually in the output file "lensparams.lens".
    The .lens file is XML format. The schema definition is provided as part of the CamCal tool.

    @param filename Path and filename of the calibration file (usually .lens suffix)
    @param params Must point to an existing smCameraLensParams struct (cannot be 0) 
    @return @ref smReturnCode "SM_API_OK" if the load occurred ok. */
...
/*! @brief Set the lens-parameters of the camera.
    ...
    maximum and minimum tracking ranges to make sense. If the lens parameters are not set, the camera is configured
    to use an approximate lens configuration, which may not be suitable for some applications.
    ...
    @return @ref smReturnCode "SM_API_FAIL_INVALID_ARGUMENT" @a sm_camera_handle is null or invalid or @a lens_params is null, or contains invalid data. */
...
/*! @brief Determines if the camera is reading from a file and end-of-file is reached.

    @param camera_handle A valid camera handle.
    @param eof Pointer to an existing smBool. Set to SM_API_TRUE if the camera is reading from a file, is not looping playback, and end of file has been reached.
    @return @ref smReturnCode "SM_API_OK" if the function completed successfully. */
SM_API(smReturnCode) smCameraEOF(smCameraHandle camera_handle, smBool *eof);

/*! @brief Loads camera extrinsic parameters from a file to define a world-coordinate frame.

    Extrinsic parameters are used to define the position and orientation of the 
    camera relative to a world-coordinate frame.

    By default, camera and world-coordinate frames are the same.

    Setting extrinsic parameters defines the position and orientation 
    of the camera in a world-coordinate frame and will alter the 3d measurements 
    from faceAPI to be provided relative to the world-coordinate frame.

    This function loads extrinsic parameters from an XML file. The XML schema is
    defined in CamCal\schema\transform3dparameters_v1.0.xsd.

    To obtain the file, perform an extrinsic camera calibration using CamCal 
    with the /w2c command-line argument. This produces a file "w2cparams.rt" 
    which can be loaded with this function.

    For more information on how to obtain and define extrinsic parameters, 
    please refer to the CamCal user guide. CamCal is provided as a development 
    tool in the faceAPI distribution under the API/tools folder.

    @param camera_handle A valid camera handle.
    @param filename Path and filename of the calibration file.
    @return @ref smReturnCode "SM_API_OK" if the load occurred ok. */
SM_API(smReturnCode) smCameraLoadW2CTransformFile(smCameraHandle camera_handle, smStringHandle filename);

/*! @brief Convenience function to supplement smCameraLoadW2CTransformFile(), 
    where loading from a file is not practical and the parameters need to be
    loaded from memory.

    @see smCameraLoadW2CTransformFile()
    ...
    @param camera_handle A valid camera handle.
    @param w2c_transform Transform that converts from world to camera 
    coordinates. Obtain this from the values in the w2cparams.rt file that is
    produced by camcal.
    @return @ref smReturnCode "SM_API_OK" if the load occurred ok. */
SM_API(smReturnCode) smCameraSetW2CTransform(smCameraHandle camera_handle, smTransform3d w2c_transform);

//SM_API(smReturnCode) smCameraSetC2WTransform(smCameraHandle camera_handle, );
//SM_API(smReturnCode) smCameraSetWorldCoordinateFramePosition(smCameraHandle camera_handle, smPos3d world_position);
//SM_API(smReturnCode) smCameraSetWorldCoordinateFrameRotation(smCameraHandle camera_handle, smRotEuler3 world_rotation);

/*! @brief Set the rotation orientation of the camera around it's Z-axis.

    This function is commonly used when the camera needs to be rotated 90 or 180 degrees
    due to the mounting situation of the camera relative to the upright head position.

    For example, if the camera delivers images 640x480 but the range of head positions 
    is larger vertically than horizontally (perhaps due to a variable height seat), then
    rotating the camera 90 degrees may offer better tracking coverage for that situation.
    In that case calling this function with a value of SM_API_CAMERA_ROTATE_90 will
    compensate for this rotation. 
    
    @note If the camera is rotated, the lens-parameters for the camera should always be 
    configured as if the camera has no rotation applied.

    @note Performance warning: rotating the image in realtime can incur a significant memory and CPU load.

    @param camera_handle A valid camera handle.
    @param rotation Rotation amount - one of four possible enum values.
    @return @ref smReturnCode "SM_API_OK" if the function completed successfully. 
    @see
    - smCameraGetRotation() */
SM_API(smReturnCode) smCameraSetRotation(smCameraHandle camera_handle, smCameraRotation rotation);

/*! @brief Get the rotation orientation of the camera around it's Z-axis.

    The default rotation is 0 degrees.

    @param camera_handle A valid camera handle.
    @param rotation Pointer to an existing variable of type smCameraRotation. Value is set to one of four possible enum values.
    @return @ref smReturnCode "SM_API_OK" if the function completed successfully. 
    @see
    - smCameraSetRotation() */
SM_API(smReturnCode) smCameraGetRotation(smCameraHandle camera_handle, smCameraRotation *rotation);
