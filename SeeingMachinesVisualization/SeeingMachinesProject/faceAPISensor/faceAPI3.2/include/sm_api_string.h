...
/*! @file
    Defines API string routines. All functions in this file are reentrant. */
#ifdef __cplusplus
extern "C"
{
#endif
...
/*! @brief Set buf to point to the the strings internal UNICODE memory buffer, which is 0 terminated.
    ...
    @return @ref smReturnCode "SM_API_OK" if the function completed successfully. */
SM_API(smReturnCode) smStringGetBufferW(smStringHandle string, wchar_t **buf, int *buf_len);
...
/*! @brief Writes a unicode UTF-16 0 terminated string into the buffer.
    ...
    @return @ref smReturnCode "SM_API_OK" if the function completed successfully. */
SM_API(smReturnCode) smStringWriteBufferW(smStringHandle string, wchar_t *buf, int buf_len);
